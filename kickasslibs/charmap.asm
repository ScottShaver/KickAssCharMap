...
CMDrawMapWindowed: {
        ...
        ldy cm_CurrentCharScrollYPosition       // instead of starting at 0 we start at how far down we are scrolled
        sty cm_CurrentMapCharY                  // store the map char y position we are going to read from
        lda #CM_DISPLAYED_MAP_Y                 // start over at the left of the on screen map area
        sta cm_CurrentDisplayY                  // store the screen Y position that the next char needs to go on

drawmap_windowed_row:
        jsr MoveMapAndPrintPointersForNewRow    // reset everything for the new row
drawmap_windowed_col:
        //--------------------------------------------------------------------------------------------------
        // draw next character to screen char memory and its corresponding color to the screen clr memory
        //--------------------------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------------------------------
        // we have put the character on the screen and set the color, now move the relavant pointers forward
        // one byte each for the cm_CurrentMapCharPointer, cm_CurrentScreenCharColorPointer, and cm_CurrentScreenCharPointer
        //--------------------------------------------------------------------------------------------------
...
!:
        ...
        //--------------------------------------------------------------------------------------------------
        // store the map char x position we are going to read from next
        //--------------------------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------------------------------
        // check if we have reached the last column of the map display
        //--------------------------------------------------------------------------------------------------
        a8b cm_CurrentCharScrollXPosition : #CM_DISPLAYED_MAP_CHAR_WIDTH
        cpx cm_calcTemp2                // have we reached the last display column?
        bne drawmap_windowed_col        // if we have not reached the last column, continue the column loop
        ...
        //--------------------------------------------------------------------------------------------------
        // store the map char Y position we are going to read from next
        //--------------------------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------------------------------
        a8b cm_CurrentCharScrollYPosition : #CM_DISPLAYED_MAP_CHAR_HEIGHT
        cpy cm_calcTemp2                // have we reached the last display row?
        bne drawmap_windowed_row        // if we have not reached the last row, continue the row loop
        rts

//----------------------------------------------------------------------------------------------------------------------------
// This subroutine moves the map, char and color pointers for a new row
//----------------------------------------------------------------------------------------------------------------------------
MoveMapAndPrintPointersForNewRow:
        // Starting a new row set the X values to the start of the row
        lda cm_CurrentCharScrollXPosition               // instead of starting at 0 we start at how far right we are scrolled
        ...
        // move the cm_CurrentMapCharPointer to the start of the correct row
...
!:
        ...
        bcs !+                                          // Branch if carry clear (no borrow from low byte), faster than doing the next 3 instructions
        // the carry was not set, so we need to handle the borrow from the low byte by subtracting from the high byte
...
!:
skipsub1:
        // move the cm_CurrentScreenCharPointer to the start of the correct row on screen
...
skipsub2:
        ...
        // memory offset, from the screen memory, to the cm_CurrentScreenCharPointer because they are the exact 
        // same size
        clc
        lda cm_CurrentScreenCharPointer                 // Load pointer low byte
        adc #<VIC_SCREEN_COLOR_MEMORY_OFFSET            // Add product low byte
        sta cm_CurrentScreenCharColorPointer            // Store back to pointer low byte
        lda cm_CurrentScreenCharPointer+1               // Load pointer high byte
        adc #>VIC_SCREEN_COLOR_MEMORY_OFFSET            // Add 0 high byte (plus any carry)
